...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        if ($driver === 'pgsql') {
            DB::statement("ALTER TABLE memories DROP CONSTRAINT IF EXISTS memories_scope_check");
            DB::statement("ALTER TABLE memories ADD CONSTRAINT memories_scope_check CHECK (scope IN ('circle', 'direct', 'private', 'public', 'connections'))");
            return;
        }

        if ($driver === 'mysql') {
            DB::statement("ALTER TABLE memories MODIFY COLUMN scope ENUM('circle', 'direct', 'private', 'public', 'connections') NOT NULL");
        }
    }
};
